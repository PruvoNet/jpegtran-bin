...
// jpegtran has no clean `--version`. `-h` prints help to stderr and exits 1
// on every libjpeg-turbo build — but only after dlopen+main resolved, so a
// controlled exit code is enough to prove the binary actually ran.
function binaryRuns() {
	if (!fs.existsSync(BINARY_PATH)) {
		return false;
	}

	const result = spawnSync(BINARY_PATH, ['-h'], {stdio: 'ignore'});
...
async function buildFromSource() {
	...
	const source = path.join(PACKAGE_ROOT, 'vendor', 'source', 'libjpeg-turbo-1.5.1.tar.gz');
	...
	await binBuild.file(source, [
		// Touch autotools-generated files so their mtimes are >= configure.ac
		// (prevents make from trying to regenerate them without autoreconf).
